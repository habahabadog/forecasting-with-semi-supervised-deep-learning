...
class CascadeSRModel(nn.Module):
    def __init__(self, base_model=RCANtime(in_chan=8, out_chan=44, n_resgroups=2, n_resblocks=2, n_feats=32, reduction=16, scale=1), base_model2=RCAN(in_chan=4, out_chan=4, n_resgroups=2, n_resblocks=2, n_feats=96, reduction=16, scale=5)):
        super(CascadeSRModel, self).__init__()
        self.base_model = base_model
        self.base_model2 = base_model2

    def dynamic_timepoint_processing(self, x1, x2, num_timepoints = 11):
        # 处理并合并每一组时间点数据
        concatenated_timepoints = []
        
        for i in range(num_timepoints):
            # 计算每个时间点的通道范围
            start = i * 4
            end = (i + 1) * 4
            concatenated_timepoints.append(torch.cat([x1[:, start:end, :, :], x2[:, start:end, :, :]], dim=1))

        processed_timepoints = []
        for i in range(num_timepoints):
            start = 4 * num_timepoints - 4 * (i + 1)
            end = 4 * num_timepoints - 4 * i
            concat = concatenated_timepoints[i]
#             print(concat.shape)
            processed = self.base_model(concat)[:, start:end, :, :]
#             print(processed.shape)
    ...
    def forward(self, x):
        x1 = self.base_model(x[:,:8,:,:])
        x2 = self.base_model(x[:,4:,:,:])
        processed_timepoints = self.dynamic_timepoint_processing(x1, x2, num_timepoints = 11)

        # 将所有处理过的时间点数据平均，然后进行空间超分辨率处理
